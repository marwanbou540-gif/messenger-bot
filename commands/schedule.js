...
module.exports = {
  name: "schedule",
  aliases: ["sch", "autoreply", "auto", "timer"],
  ...
  usage: [
    ...
    "",
    "مثال:",
    "  -schedule add 30 m صباح الخير 🌅",
    "  -schedule add 2 h تذكير: التزموا بالقوانين",
  ].join("\n"),
  category: "Group",
  groupOnly: true,
  adminOnly: true,
  ...
  async execute({ api, event, args }) {
    ...
    if (sub === "list") {
      ...
      if (!ts.length) {
        return api.sendMessage(`📭 لا توجد جداول نشطة.\nأضف واحداً: ${prefix}schedule add <مقدار> <وحدة> <الرسالة>`, threadID);
      }
      ...
      msg += `└─ لإيقاف جدول: ${prefix}schedule stop <ID>`;
    ...
    if (sub === "stop") {
      ...
      if (isNaN(id)) return api.sendMessage(`❌ مثال: ${prefix}schedule stop 1`, threadID);
