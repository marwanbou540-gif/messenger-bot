 ...
 *   - Temp files cleaned up automatically (30s after send + on startup)
 */
...
const MAX_DURATION_SEC = 720;    // 12 minutes max
const MAX_FILE_BYTES   = 48 * 1024 * 1024;  // 48 MB
const USER_COOLDOWN_MS = 35_000;

// ── State ─────────────────────────────────────────────────────────────────────
const _userCooldowns = new Map();
let   _ytdlpPath     = null;   // resolved yt-dlp command/path
let   _ytdlpChecked  = false;
let   _ytdlpPromise  = null;   // in-flight check promise (deduplicate)

// ── Semaphore (download concurrency gate) ─────────────────────────────────────
class Semaphore {
  constructor(max) {
    this._max     = max;
    this._running = 0;
  ...
  acquire() {
    return new Promise(resolve => {
      ...
      if (this._running < this._max) {
        ...
        this._queue.push(() => { this._running++; resolve(release); });
      }
    });
  }
  _flush() {
...
// ── Startup: ensure tmp dir + clean stale files ───────────────────────────────
...
// ── Non-blocking HTTP GET → Buffer (handles redirects) ───────────────────────
...
// ── yt-dlp binary: find system install, then local binary, then download ──────
...
async function _resolveYtDlp() {
  // 1. System yt-dlp (installed via pip in nixpacks build phase)
  for (const cmd of ["yt-dlp", "/usr/bin/yt-dlp", "/usr/local/bin/yt-dlp"]) {
    try {
  ...
  // 2. Local binary (cached from previous download)
  ...
  // 3. Download standalone Linux binary from GitHub
  ...
  logger.success("MusicEngine", "yt-dlp binary ready");
  return (_ytdlpPath = YTDLP_LOCAL);
}

// ── Provider 1: YouTube via yt-search + yt-dlp ───────────────────────────────
async function _searchYouTube(query) {
  ...
  const videos   = (result.videos || []).filter(
    v => v.seconds && v.seconds > 15 && v.seconds < MAX_DURATION_SEC
  );
...
// ── Provider 2: iTunes Search API (30s preview) ───────────────────────────────
...
/** Search for a track. Returns track metadata. Throws on failure. */
async function search(query) {
  ...
  try { return await _searchYouTube(q); }
    ...
    if (e.message !== "no_results")
      logger.warn("MusicEngine", `YouTube search failed: ${e.message}`);
  }
  ...
  try { return await _searchItunes(q); }
    ...
    if (e.message !== "no_results")
      logger.warn("MusicEngine", `iTunes search failed: ${e.message}`);
  }

  throw new Error("لم يُعثر على نتائج لـ: " + q);
}

/** Download a track to a temp file. Returns the file path. */
async function download(track) {
  if (_sem.total >= QUEUE_MAX) {
    throw new Error("قائمة الانتظار ممتلئة، حاول بعد قليل.");
  }

  const outPath = path.join(
    TMP_DIR,
    `music_${Date.now()}_${Math.random().toString(36).slice(2)}.m4a`
  );
...
/** Delete a file safely, after a delay (default 30s). */
...
/** Check if a user is on cooldown. Returns remaining seconds (0 = ok). */
...
/** Mark the user as having just made a request. */
function markUser(senderID) {
  _userCooldowns.set(senderID, Date.now());
}

/** Engine diagnostics for -music diag command. */
